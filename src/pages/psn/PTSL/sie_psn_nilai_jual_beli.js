...
const realisasiPenggunaan = () => {
  ...
  const getData = () => {
    ...
    axios
      .post(
        `${url}ProgramStrategisNasional/PTSL/sie_psn_nilai_jual_beli?tahunAwal=${tahunAwal}&tahunAkhir=${years}`,
        temp
      )
      .then(function (response) {
  ...
  const title = " PSN Nilai Jual Beli BPHTB";
  const handlePrint = () => {
    history.push({
      pathname: "/PrintData",
      state: {
            ...
            >
              <CopyToClipboard
                text={
                  ...
                  BASE_URL.domain +
                  "/embed/" +
                  BASE_URL.path.sie_psn_nilai_jual_beli +
                  '"></iframe>'
                }
